...
\author{
	Alfredo Gomes \\ A71655\and
	Octávio Maia \\ A71369 \and
	Pedro Rocha \\ A71802 \and
	\dium
\\
...
\subsubsection{Função depth:}
Esta função permite auxiliar a função |balanced|, definida no enunciado, de maneira a retornar a profundidade de uma |LTree|, isto é, quantos níveis tem a árvore.
Para isso tivemos de definir esta função usando o catamorfismo em |LTree|. Concluimos que o resultado ou ia ser 1 (one) ou então teriamos de descobrir onde (na sub-árvore da esquerda ou direita) existe o máximo de níveis e depois somar um a esse número devolvendo o resultado final (|succ.uncurry(max)|)
\begin{code}
depth :: LTree a -> Integer
depth = cataLTree (either (one) (succ.uncurry(max)))
\end{code}

\subsubsection{Função balance:}
Neste caso tivemos de refazer a função |balance| apenas numa linha recorrendo a funções da biblioteca |LTree|. 
Para isso tivemos de fazer o anamorfismo da |lsplit| que permite organizar a árvore de maneira a ela ficar balanceada após receber a lista com os elementos da árvore. Elementos estes que são providenciados pelo catamorfismo que guarda o valor de um nodo numa lista, seguido da destruição do mesmo.
\begin{code}
balance :: LTree a -> LTree a
balance = anaLTree (lsplit) . cataLTree (either singl (uncurry (++)))
\end{code}

\subsection*{Secção \ref{sec:BTree}}
\subsubsection{Função qsplit:}
Nesta secção tivemos de começar por definir o gene qsplit em haskell. 
O gene |qsplit|, dado um par de inteiros, cujo par não pode conter números iguais a 0, devolve uma árvore gerada com base nos inteiros fornecidos. 
Se o primeiro número for menor ou igual que o segundo, a árvore é construída colocando na raiz um s |(soma de ambos os numeros a dividir por dois)|, e colocando numa sub-árvore cuja raiz é o nodo esquerdo da árvore original, o primeiro elemento do par e |s-1|, enquanto que na outra sub-árvore coloca |s+1| e o segundo elemento do par, caso contrario não é criada a árvore.
\begin{code}
qsplit :: Integral a => (a, a) -> Either () (a, ((a, a), (a, a)))
qsplit (x,y) =  if (x==0 || y==0) then Left ()
	            else let s = div(x+y) 2
                     in if x <= y  then Right (s, ((x, (s - 1)),((s + 1) , y)))
                                   else Left ()
\end{code}

\subsection*{Secção \ref{sec:SList}}
\subsubsection{Funções base da SList:}
Este conjunto de funções é semelhante a outras bibliotecas usadas na disciplina, sendo estas adaptadas para a estrutura de dados |SList|.

A função |inSList| faz a separação dos construtores Sent (se a lista apenas possuir a sentinela) e Cons (se houver uma lista completa finalizada por uma sentinela), a função |outSList| faz o oposto.

A função |recSList| assegura a recursividade das listas, |anaSList| constroi uma |SList|, a |cataSList| devolve um valor depois de fazer a recursividade, e por fim a |hyloSList| é a composição das funções cataSList após a |anaSList|.
...
\subsubsection{Função mgen:}
A função |mgen| recebe um par de listas ordenadas. Caso uma das listas seja vazia devolve a outra lista. Caso contrário devolve um tuplo em que o primeiro elemento é o maior da comparação entre as cabeças das listas, sendo o segundo um tuplo de listas identicas às recebidas como parâmetro excepto que irá ter o menor elemento da mesma removido.
\begin{code}
mgen :: Ord a => ([a], [a]) -> Either [a] (a, ([a], [a]))
mgen (l, []) = Left l
mgen ([], r) = Left r
mgen ((x:xs),(y:ys)) = if(x<=y) then Right(x,(xs,y:ys))
					   else Right(y,(x:xs,ys))
\end{code}

\subsection*{Secção \ref{sec:sierp}}
\subsubsection{Funções base de TLTree:}
Este conjunto de funções é semelhante a outras bibliotecas usadas na disciplina, sendo estas adaptadas para a estrutura de dados |TLTree|. A explicação destas funções são semelhantes às apresentadas anteriormente para a biblioteca SList.
\begin{code}
inTLTree = either L N

outTLTree (L d) = Left d
outTLTree (N (a,(b,c))) = (Right (a,(b,c)))
...
\subsubsection{Funções tips, count, depth e inv da TLTree:}
A função |tipsTLTree| devolve um lista com todos os elementos de uma |TLTree|. Para isto, aplicamos um catamorfismo que irá percorrer toda a árvore recursivamente, dando uma lista apenas com um elemento (|singl|) ou concatena as três sub-árvores atravês do conc.

A função |countTLTree| conta quantos nós tem uma árvore. Aplicando o catamorfismo a função retorna 1 (|const 1|) se a árvore apresentar só um nó ou retorna a soma de todos os nós presentes nas três sub-árvores caso contrário.

A função |depthTLTree| calcula a profundidade da árvore |TLTree|. 

Por último, a função |invTLTree| inverte uma |TLTree|. Para fazer esta função, tivemos de fazer uma função auxiliar |swapT| que aplica o swap a 3 elementos (x,(y,z)) = (z,(y,x))
\begin{code}
tipsTLTree :: TLTree b -> [b ]
tipsTLTree  = cataTLTree (either singl conc)
             where conc(l,(x,y)) = l ++ x ++ y

countTLTree :: TLTree b -> Int
countTLTree = cataTLTree (either (const 1) add)
              where add(l,(x,y)) = l + x + y
...
invTLTree = cataTLTree (inTLTree . (id -|- swapT))
            where swapT :: (a,(b,c)) -> (c,(b,a)) 
                  swapT (x,(y,z)) = (z,(y,x))

\end{code}

\subsubsection{Funções gera, geraSierp e apresentaSierp:}
A função |gera| recebe como parâmetros o triangulo original e um inteiro que simboliza o número de divisões que iremos fazer (o número de recursões da função).

A função |geraSierp| utiliza o anamorfismo após a junção do |Tri| e |Int| num par, fazendo a recursividade na função |gera|.

A função |apresentaSierp| recebe como parâmetro uma |TLTree| e devolve uma lista de elementos |Tri|.

\begin{code}     

gera :: (Tri,Int) -> Either Tri ((Tri,Int),(((Tri,Int), (Tri,Int))))
gera (t,0) = Left (t)
gera (((x,y),s),n)  = let s' = div s 2
                       in Right ((((x,y),s'),n-1), ((((x+s',y), s'), n-1), (((x,y+s'), s'),n-1)))

geraSierp :: Tri -> Int -> TLTree Tri
geraSierp = curry (anaTLTree gera)

apresentaSierp :: TLTree Tri -> [Tri]
apresentaSierp = cataTLTree (either singl (uncurry (++) . (id >< uncurry (++))))
\end{code}

\subsubsection{Função rep e draw:}
A função draw permite gerar a visualização do triângulo, num browser.
A função |rep| tem como objetivo gerar o triângulo de Sierpinski, usando a função |gerar| , função esta que irá transformar a |TLTree| numa lista usando a função |apresentaSierp|. No final desta conversão a lista irá ser convertida num ficheiro |html| pela função |drawTriangle|, utilizando por fim a função |finalize| para ser possível ver o ficheiro |html| num browser.

\begin{code}
draw = render html where
       html = rep dados

rep = finalize. concat . (map drawTriangle) . apresentaSierp. gerar 
           where gerar (tri, num) = geraSierp tri (fromIntegral num)
\end{code}
...
A função |gene| executa um |either perderStop perderPalavraLista| que devolve a probabilidade de se perder apenas a última palavra, "stop", a partir da função |perderStop|. Devolve também a probabilidade de se perder qualquer uma das palavras que fazem parte da frase, usando para este fim a função |perderPalavraLista|.

A função |gene| é utilizada na função |transmitir = pcataList gene|.

\begin{code}

gene = either perderStop perderPalavraLista
	
	   where 
			perderPalavraLista (a,b) = D [((a:b),0.95),(b,0.05)]
			perderStop a = (D[([],0.1),(["stop"],0.9)])
                

\end{code}

Obtivemos o seguinte output ao executar o comando |transmitir (words "Vamos atacar hoje")|:
...
\begin{itemize}
\item Existe |77.2%| de probabilidade de transmitir a frase completa.
\item Existe |8.6%| de probabilidade de transmitir todos os elementos da frase excepto o |stop|.
\item Existe |4.1%| de probabilidade de perder qualquer uma das seguintes palavras: |vamos|,|atacar| ou |hoje|, embora nunca perdamos o |stop|.

\end{itemize}


\subsection*{Secção \ref{sec:parBTreeMap}}
A função |parBTreeMap| tem como objetivo aplicar a função previamente definida |rpar| a todos os elementos de uma |BTree| através da invocação da função |parBTreeMap| recursiva pelo ciclo |do|.

\begin{code}
parBTreeMap t (Empty) = return Empty;
parBTreeMap t (Node(r,(e,d))) = do
							  r' <- rpar(t r) 
							  e' <- parBTreeMap t e
							  d' <- parBTreeMap t d
...
Obtivemos os seguinte resultados após executar a versão sequencial e paralela, respetivamente:

\begin{verbatim}
  Total   time    1.33s  (  0.84s elapsed)
  Total   time    0.70s  (  0.38s elapsed)
\end{verbatim}

  Em suma, a versão paralela é cerca de 2.21x mais rápida |((frac 0.84 0.38))|
que a sequencial.
