...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{render}\;\Varid{html}\mathrel{=}\mathbf{do}\;\{\mskip1.5mu \Varid{writeFile}\;\text{\tt \char34 \char95 .html\char34}\;\Varid{html};\Varid{system}\;\text{\tt \char34 firefox~\char95 .html\char34}\mskip1.5mu\}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\\
\>[6]{}\hsindent{3}{}\<[9]%
\>[9]{}\Varid{execExempSeq}\mathrel{=}\underline{(\Varid{putStrLn}\comp \Varid{show}\comp (\Varid{fmap}\;\Varid{fib})\mathbin{\$}\Varid{abpe}\;(\mathrm{20},\mathrm{30}))}{}\<[E]%
\\
\>[6]{}\hsindent{3}{}\<[9]%
\>[9]{}\Varid{execExempPar}\mathrel{=}\underline{(\Varid{putStrLn}\comp \Varid{show}\comp \Varid{runEval}\comp (\Varid{parBTreeMap}\;\Varid{fib})\mathbin{\$}\Varid{abpe}\;(\mathrm{20},\mathrm{30}))}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
Esta função permite auxiliar a função \ensuremath{\Varid{balanced}}, definida no enunciado, de maneira a retornar a profundidade de uma \ensuremath{\Conid{LTree}}, isto é, quantos níveis tem a árvore.
Para isso tivemos de definir esta função usando o catamorfismo em \ensuremath{\Conid{LTree}}. Concluimos que o resultado ou ia ser 1 (one) ou então teriamos de descobrir onde (na sub-árvore da esquerda ou direita) existe o máximo de níveis e depois somar um a esse número devolvendo o resultado final (\ensuremath{\Varid{succ}\comp \uncurry{(\Varid{max})}})
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
...
Neste caso tivemos de refazer a função \ensuremath{\Varid{balance}} apenas numa linha recorrendo a funções da biblioteca \ensuremath{\Conid{LTree}}. 
Para isso tivemos de fazer o anamorfismo da \ensuremath{\Varid{lsplit}} que permite organizar a árvore de maneira a ela ficar balanceada após receber a lista com os elementos da árvore. Elementos estes que são providenciados pelo catamorfismo que guarda o valor de um nodo numa lista, seguido da destruição do mesmo.
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{balance}\mathbin{::}\Conid{LTree}\;\Varid{a}\to \Conid{LTree}\;\Varid{a}{}\<[E]%
\\
\>[B]{}\Varid{balance}\mathrel{=}\Varid{anaLTree}\;(\Varid{lsplit})\comp \Varid{cataLTree}\;\alt{\Varid{singl}}{\uncurry{(\plus )}}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
O gene \ensuremath{\Varid{qsplit}}, dado um par de inteiros, cujo par não pode conter números iguais a 0, devolve uma árvore gerada com base nos inteiros fornecidos. 
Se o primeiro número for menor ou igual que o segundo, a árvore é construída colocando na raiz um s \ensuremath{(\Varid{soma}\;\Varid{de}\;\Varid{ambos}\;\Varid{os}\;\Varid{numeros}\;\Varid{a}\;\Varid{dividir}\;\Varid{por}\;\Varid{dois})}, e colocando numa sub-árvore cuja raiz é o nodo esquerdo da árvore original, o primeiro elemento do par e \ensuremath{\Varid{s}\mathbin{-}\mathrm{1}}, enquanto que na outra sub-árvore coloca \ensuremath{\Varid{s}\mathbin{+}\mathrm{1}} e o segundo elemento do par, caso contrario não é criada a árvore.
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
...
\\
\>[B]{}\Varid{qsplit}\;(\Varid{x},\Varid{y})\mathrel{=}{}\<[17]%
\>[17]{}\mathbf{if}\;(\Varid{x}\equiv \mathrm{0}\mathrel{\vee}\Varid{y}\equiv \mathrm{0})\;\mathbf{then}\;i_1\;(){}\<[E]%
\\
\>[17]{}\hsindent{4}{}\<[21]%
\>[21]{}\mathbf{else}\;\mathbf{let}\;\Varid{s}\mathrel{=}\Varid{x}\mathbin{+}\Varid{y}\div \mathrm{2}{}\<[E]%
\\
\>[21]{}\hsindent{1}{}\<[22]%
\>[22]{}\mathbf{in}\;\mathbf{if}\;\Varid{x}\leq \Varid{y}\;{}\<[36]%
\>[36]{}\mathbf{then}\;i_2\;(\Varid{s},((\Varid{x},(\Varid{s}\mathbin{-}\mathrm{1})),((\Varid{s}\mathbin{+}\mathrm{1}),\Varid{y}))){}\<[E]%
\\
\>[36]{}\mathbf{else}\;i_1\;(){}\<[E]%
\ColumnHook
\end{hscode}\resethooks

\subsection*{Secção \ref{sec:SList}}
\subsubsection{Funções base da SList:}
Este conjunto de funções é semelhante a outras bibliotecas usadas na disciplina, sendo estas adaptadas para a estrutura de dados \ensuremath{\Conid{SList}}.

A função \ensuremath{\Varid{inSList}} faz a separação dos construtores Sent (se a lista apenas possuir a sentinela) e Cons (se houver uma lista completa finalizada por uma sentinela), a função \ensuremath{\Varid{outSList}} faz o oposto.

A função \ensuremath{\Varid{recSList}} assegura a recursividade das listas, \ensuremath{\Varid{anaSList}} constroi uma \ensuremath{\Conid{SList}}, a \ensuremath{\Varid{cataSList}} devolve um valor depois de fazer a recursividade, e por fim a \ensuremath{\Varid{hyloSList}} é a composição das funções cataSList após a \ensuremath{\Varid{anaSList}}.
...
\\
\>[B]{}\Varid{outSList}\;(\Conid{Cons}\;(\Varid{b},\Varid{a}))\mathrel{=}i_2\;(\Varid{b},\Varid{a}){}\<[E]%
\\[\blanklineskip]%
...
\\
\>[B]{}\Varid{anaSList}\;\Varid{f}\mathrel{=}\Varid{inSList}\comp (\Varid{recSList}\;(\Varid{anaSList}\;\Varid{f}))\comp \Varid{f}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{cataSList}\mathbin{::}(\Conid{Either}\;\Varid{b}\;(\Varid{a},\Varid{d})\to \Varid{d})\to \Conid{SList}\;\Varid{a}\;\Varid{b}\to \Varid{d}{}\<[E]%
\\
\>[B]{}\Varid{cataSList}\;\Varid{a}\mathrel{=}\Varid{a}\comp (\Varid{recSList}\;(\Varid{cataSList}\;\Varid{a}))\comp \Varid{outSList}{}\<[E]%
\\[\blanklineskip]%
...
\\
\>[B]{}\Varid{hyloSList}\;\Varid{a}\;\Varid{c}\mathrel{=}\Varid{cataSList}\;\Varid{a}\comp \Varid{anaSList}\;\Varid{c}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks

\subsubsection{Função mgen:}
A função \ensuremath{\Varid{mgen}} recebe um par de listas ordenadas. Caso uma das listas seja vazia devolve a outra lista. Caso contrário devolve um tuplo em que o primeiro elemento é o maior da comparação entre as cabeças das listas, sendo o segundo um tuplo de listas identicas às recebidas como parâmetro excepto que irá ter o menor elemento da mesma removido.
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
...
\\
\>[B]{}\Varid{mgen}\;(\Varid{l},[\mskip1.5mu \mskip1.5mu])\mathrel{=}i_1\;\Varid{l}{}\<[E]%
\\
\>[B]{}\Varid{mgen}\;([\mskip1.5mu \mskip1.5mu],\Varid{r})\mathrel{=}i_1\;\Varid{r}{}\<[E]%
\\
\>[B]{}\Varid{mgen}\;((\Varid{x}\mathbin{:}\Varid{xs}),(\Varid{y}\mathbin{:}\Varid{ys}))\mathrel{=}\mathbf{if}\;(\Varid{x}\leq \Varid{y})\;\mathbf{then}\;i_2\;(\Varid{x},(\Varid{xs},\Varid{y}\mathbin{:}\Varid{ys})){}\<[E]%
\\
\>[B]{}\hsindent{44}{}\<[44]%
\>[44]{}\mathbf{else}\;i_2\;(\Varid{y},(\Varid{x}\mathbin{:}\Varid{xs},\Varid{ys})){}\<[E]%
\ColumnHook
\end{hscode}\resethooks

\subsection*{Secção \ref{sec:sierp}}
\subsubsection{Funções base de TLTree:}
Este conjunto de funções é semelhante a outras bibliotecas usadas na disciplina, sendo estas adaptadas para a estrutura de dados \ensuremath{\mathsf{TLTree}}. A explicação destas funções são semelhantes às apresentadas anteriormente para a biblioteca SList.
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{inTLTree}\mathrel{=}\alt{\Conid{L}}{\Conid{N}}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{outTLTree}\;(\Conid{L}\;\Varid{d})\mathrel{=}i_1\;\Varid{d}{}\<[E]%
\\
...
A função \ensuremath{\Varid{tipsTLTree}} devolve um lista com todos os elementos de uma \ensuremath{\mathsf{TLTree}}. Para isto, aplicamos um catamorfismo que irá percorrer toda a árvore recursivamente, dando uma lista apenas com um elemento (\ensuremath{\Varid{singl}}) ou concatena as três sub-árvores atravês do conc.

A função \ensuremath{\Varid{countTLTree}} conta quantos nós tem uma árvore. Aplicando o catamorfismo a função retorna 1 (\ensuremath{\underline{\mathrm{1}}}) se a árvore apresentar só um nó ou retorna a soma de todos os nós presentes nas três sub-árvores caso contrário.

A função \ensuremath{\Varid{depthTLTree}} calcula a profundidade da árvore \ensuremath{\mathsf{TLTree}}. 

Por último, a função \ensuremath{\Varid{invTLTree}} inverte uma \ensuremath{\mathsf{TLTree}}. Para fazer esta função, tivemos de fazer uma função auxiliar \ensuremath{\Varid{swapT}} que aplica o swap a 3 elementos (x,(y,z)) = (z,(y,x))
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
...
\\
\>[B]{}\Varid{tipsTLTree}{}\<[13]%
\>[13]{}\mathrel{=}\Varid{cataTLTree}\;\alt{\Varid{singl}}{\mathsf{conc}}{}\<[E]%
\\
\>[13]{}\hsindent{1}{}\<[14]%
\>[14]{}\mathbf{where}\;\mathsf{conc}\;(\Varid{l},(\Varid{x},\Varid{y}))\mathrel{=}\Varid{l}\plus \Varid{x}\plus \Varid{y}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{countTLTree}\mathbin{::}\mathsf{TLTree}\;\Varid{b}\to \Conid{Int}{}\<[E]%
\\
\>[B]{}\Varid{countTLTree}\mathrel{=}\Varid{cataTLTree}\;\alt{\underline{\mathrm{1}}}{\Varid{add}}{}\<[E]%
\\
\>[B]{}\hsindent{15}{}\<[15]%
\>[15]{}\mathbf{where}\;\Varid{add}\;(\Varid{l},(\Varid{x},\Varid{y}))\mathrel{=}\Varid{l}\mathbin{+}\Varid{x}\mathbin{+}\Varid{y}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{depthTLTree}\mathbin{::}\mathsf{TLTree}\;\Varid{b}\to \Conid{Int}{}\<[E]%
\\
\>[B]{}\Varid{depthTLTree}\mathrel{=}{}\<[16]%
\>[16]{}\Varid{cataTLTree}\;\alt{\underline{(\mathrm{1})}}{\Varid{succ}\comp \uncurry{(\Varid{max})}\comp (\Varid{id}\times\uncurry{(\Varid{max})})}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{invTLTree}\mathbin{::}\mathsf{TLTree}\;\Varid{b}\to \mathsf{TLTree}\;\Varid{b}{}\<[E]%
\\
\>[B]{}\Varid{invTLTree}\mathrel{=}\Varid{cataTLTree}\;(\Varid{inTLTree}\comp (\Varid{id}+\Varid{swapT})){}\<[E]%
\\
\>[B]{}\hsindent{13}{}\<[13]%
\>[13]{}\mathbf{where}\;\Varid{swapT}\mathbin{::}(\Varid{a},(\Varid{b},\Varid{c}))\to (\Varid{c},(\Varid{b},\Varid{a})){}\<[E]%
\\
\>[13]{}\hsindent{6}{}\<[19]%
\>[19]{}\Varid{swapT}\;(\Varid{x},(\Varid{y},\Varid{z}))\mathrel{=}(\Varid{z},(\Varid{y},\Varid{x})){}\<[E]%
\ColumnHook
\end{hscode}\resethooks

\subsubsection{Funções gera, geraSierp e apresentaSierp:}
A função \ensuremath{\Varid{gera}} recebe como parâmetros o triangulo original e um inteiro que simboliza o número de divisões que iremos fazer (o número de recursões da função).

A função \ensuremath{\Varid{geraSierp}} utiliza o anamorfismo após a junção do \ensuremath{\Conid{Tri}} e \ensuremath{\Conid{Int}} num par, fazendo a recursividade na função \ensuremath{\Varid{gera}}.

A função \ensuremath{\Varid{apresentaSierp}} recebe como parâmetro uma \ensuremath{\mathsf{TLTree}} e devolve uma lista de elementos \ensuremath{\Conid{Tri}}.
...
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{gera}\mathbin{::}(\Conid{Tri},\Conid{Int})\to \Conid{Either}\;\Conid{Tri}\;((\Conid{Tri},\Conid{Int}),(((\Conid{Tri},\Conid{Int}),(\Conid{Tri},\Conid{Int})))){}\<[E]%
\\
\>[B]{}\Varid{gera}\;(\Varid{t},\mathrm{0})\mathrel{=}i_1\;(\Varid{t}){}\<[E]%
\\
\>[B]{}\Varid{gera}\;(((\Varid{x},\Varid{y}),\Varid{s}),\Varid{n}){}\<[21]%
\>[21]{}\mathrel{=}\mathbf{let}\;\Varid{s'}\mathrel{=}\Varid{s}\div \mathrm{2}{}\<[E]%
\\
\>[21]{}\hsindent{3}{}\<[24]%
\>[24]{}\mathbf{in}\;i_2\;((((\Varid{x},\Varid{y}),\Varid{s'}),\Varid{n}\mathbin{-}\mathrm{1}),((((\Varid{x}\mathbin{+}\Varid{s'},\Varid{y}),\Varid{s'}),\Varid{n}\mathbin{-}\mathrm{1}),(((\Varid{x},\Varid{y}\mathbin{+}\Varid{s'}),\Varid{s'}),\Varid{n}\mathbin{-}\mathrm{1}))){}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{geraSierp}\mathbin{::}\Conid{Tri}\to \Conid{Int}\to \mathsf{TLTree}\;\Conid{Tri}{}\<[E]%
\\
\>[B]{}\Varid{geraSierp}\mathrel{=}\Varid{curry}\;(\Varid{anaTLTree}\;\Varid{gera}){}\<[E]%
\\[\blanklineskip]%
...
A função \ensuremath{\Varid{rep}} tem como objetivo gerar o triângulo de Sierpinski, usando a função \ensuremath{\Varid{gerar}} , função esta que irá transformar a \ensuremath{\mathsf{TLTree}} numa lista usando a função \ensuremath{\Varid{apresentaSierp}}. No final desta conversão a lista irá ser convertida num ficheiro \ensuremath{\Varid{html}} pela função \ensuremath{\Varid{drawTriangle}}, utilizando por fim a função \ensuremath{\Varid{finalize}} para ser possível ver o ficheiro \ensuremath{\Varid{html}} num browser.
...
\\[\blanklineskip]%
\>[B]{}\Varid{rep}\mathrel{=}\Varid{finalize}\comp \Varid{concat}\comp (\Varid{map}\;\Varid{drawTriangle})\comp \Varid{apresentaSierp}\comp \Varid{gerar}{}\<[E]%
\\
\>[B]{}\hsindent{12}{}\<[12]%
\>[12]{}\mathbf{where}\;\Varid{gerar}\;(\Varid{tri},\Varid{num})\mathrel{=}\Varid{geraSierp}\;\Varid{tri}\;(\Varid{fromIntegral}\;\Varid{num}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
A função \ensuremath{\Varid{gene}} executa um \ensuremath{\alt{\Varid{perderStop}}{\Varid{perderPalavraLista}}} que devolve a probabilidade de se perder apenas a última palavra, "stop", a partir da função \ensuremath{\Varid{perderStop}}. Devolve também a probabilidade de se perder qualquer uma das palavras que fazem parte da frase, usando para este fim a função \ensuremath{\Varid{perderPalavraLista}}.

A função \ensuremath{\Varid{gene}} é utilizada na função \ensuremath{\Varid{transmitir}\mathrel{=}\Varid{pcataList}\;\Varid{gene}}.
...
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{gene}\mathrel{=}\alt{\Varid{perderStop}}{\Varid{perderPalavraLista}}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\hsindent{12}{}\<[12]%
\>[12]{}\mathbf{where}{}\<[E]%
\\
\>[12]{}\hsindent{13}{}\<[25]%
\>[25]{}\Varid{perderPalavraLista}\;(\Varid{a},\Varid{b})\mathrel{=}\Conid{D}\;[\mskip1.5mu ((\Varid{a}\mathbin{:}\Varid{b}),\mathrm{0.95}),(\Varid{b},\mathrm{0.05})\mskip1.5mu]{}\<[E]%
\\
\>[12]{}\hsindent{13}{}\<[25]%
\>[25]{}\Varid{perderStop}\;\Varid{a}\mathrel{=}(\Conid{D}\;[\mskip1.5mu ([\mskip1.5mu \mskip1.5mu],\mathrm{0.1}),([\mskip1.5mu \text{\tt \char34 stop\char34}\mskip1.5mu],\mathrm{0.9})\mskip1.5mu]){}\<[E]%
\ColumnHook
\end{hscode}\resethooks

Obtivemos o seguinte output ao executar o comando \ensuremath{\Varid{transmitir}\;(\Varid{words}\;\text{\tt \char34 Vamos~atacar~hoje\char34})}:
...
\begin{itemize}
\item Existe \ensuremath{\mathrm{77.2}\mathbin{\%}} de probabilidade de transmitir a frase completa.
\item Existe \ensuremath{\mathrm{8.6}\mathbin{\%}} de probabilidade de transmitir todos os elementos da frase excepto o \ensuremath{\Varid{stop}}.
\item Existe \ensuremath{\mathrm{4.1}\mathbin{\%}} de probabilidade de perder qualquer uma das seguintes palavras: \ensuremath{\Varid{vamos}},\ensuremath{\Varid{atacar}} ou \ensuremath{\Varid{hoje}}, embora nunca perdamos o \ensuremath{\Varid{stop}}.

\end{itemize}


\subsection*{Secção \ref{sec:parBTreeMap}}
A função \ensuremath{\Varid{parBTreeMap}} tem como objetivo aplicar a função previamente definida \ensuremath{\Varid{rpar}} a todos os elementos de uma \ensuremath{\Conid{BTree}} através da invocação da função \ensuremath{\Varid{parBTreeMap}} recursiva pelo ciclo \ensuremath{\mathbf{do}}.

\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{59}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{parBTreeMap}\;\Varid{t}\;(\Conid{Empty})\mathrel{=}\Varid{return}\;\Conid{Empty};{}\<[E]%
\\
\>[B]{}\Varid{parBTreeMap}\;\Varid{t}\;(\Conid{Node}\;(\Varid{r},(\Varid{e},\Varid{d})))\mathrel{=}\mathbf{do}{}\<[E]%
\\
\>[B]{}\hsindent{59}{}\<[59]%
\>[59]{}\Varid{r'}\leftarrow \Varid{rpar}\;(\Varid{t}\;\Varid{r}){}\<[E]%
\\
\>[B]{}\hsindent{59}{}\<[59]%
\>[59]{}\Varid{e'}\leftarrow \Varid{parBTreeMap}\;\Varid{t}\;\Varid{e}{}\<[E]%
\\
\>[B]{}\hsindent{59}{}\<[59]%
\>[59]{}\Varid{d'}\leftarrow \Varid{parBTreeMap}\;\Varid{t}\;\Varid{d}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\hsindent{59}{}\<[59]%
\>[59]{}\Varid{return}\;(\Conid{Node}\;(\Varid{r'},(\Varid{e'},\Varid{d'}))){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\begin{tabbing}\tt
~~~Total~~~time~~~~1\char46{}33s~~\char40{}~~0\char46{}84s~elapsed\char41{}\\
\tt ~~~Total~~~time~~~~0\char46{}70s~~\char40{}~~0\char46{}38s~elapsed\char41{}
\end{tabbing}
